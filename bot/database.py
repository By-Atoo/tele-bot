...
class Database:
    ...
    def get_top_records(self, limit=20):
        rows = self.conn.execute('SELECT * FROM records ORDER BY score DESC LIMIT ?', (limit,)).fetchall()
        return [dict(r) for r in rows]

    def get_all_records(self, order_by='score DESC'):
        allowed = {'score DESC':'score DESC','score ASC':'score ASC','duration DESC':'duration DESC',
                   'duration ASC':'duration ASC','timestamp DESC':'timestamp DESC','timestamp ASC':'timestamp ASC'}
        order_clause = allowed.get(order_by, 'score DESC')
        rows = self.conn.execute(f'SELECT * FROM records ORDER BY {order_clause}').fetchall()
        return [dict(r) for r in rows]
    ...
    def search_records_by_name(self, query):
        rows = self.conn.execute('SELECT * FROM records WHERE name LIKE ? ORDER BY score DESC', (f'%{query}%',)).fetchall()
        return [dict(r) for r in rows]

    def get_statistics(self):
        row = self.conn.execute('SELECT COUNT(*), AVG(score), MAX(score), MIN(score) FROM records').fetchone()
    ...
    def get_ai_logs(self, limit=50, offset=0, search_query=None):
        if search_query:
            try:
                ...
                rows = self.conn.execute('''SELECT * FROM ai_logs WHERE user_id = ? OR username LIKE ?
                                           ORDER BY timestamp DESC LIMIT ? OFFSET ?''',
                                        (search_id, f'%{search_query}%', limit, offset)).fetchall()
            except ValueError:
                rows = self.conn.execute('''SELECT * FROM ai_logs WHERE username LIKE ?
                                           ORDER BY timestamp DESC LIMIT ? OFFSET ?''',
                                        (f'%{search_query}%', limit, offset)).fetchall()
        else:
            rows = self.conn.execute('SELECT * FROM ai_logs ORDER BY timestamp DESC LIMIT ? OFFSET ?',
    ...
    def count_ai_logs(self, search_query=None):
        if search_query:
            try:
                search_id = int(search_query)
                row = self.conn.execute('SELECT COUNT(*) FROM ai_logs WHERE user_id = ? OR username LIKE ?',
                                       (search_id, f'%{search_query}%')).fetchone()
            except ValueError:
                row = self.conn.execute('SELECT COUNT(*) FROM ai_logs WHERE username LIKE ?',
                                       (f'%{search_query}%',)).fetchone()
        else:
    ...
    def delete_ai_logs(self):
        ...
        self.conn.commit()
        logger.info("All AI logs deleted.")

    def delete_ai_logs_by_user(self, user_id):
        self.conn.execute('DELETE FROM ai_logs WHERE user_id = ?', (user_id,))
        self.conn.commit()
    ...
    def get_all_users(self):
        rows = self.conn.execute('SELECT * FROM users ORDER BY last_seen DESC').fetchall()
        return [dict(r) for r in rows]
